...
async def test_user_flow_success(hass: HomeAssistant) -> None:
    ...
    with (
        ...
        patch("custom_components.geosphere_next.async_setup_entry", return_value=True),
    ):
        mocked.get(AROME_URL, payload=load_fixture("arome.json"))
        mocked.get(NOWCAST_URL, payload=load_fixture("nowcast.json"))
...
async def test_user_flow_out_of_domain(hass: HomeAssistant) -> None:
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": SOURCE_USER}
    )
    with aioresponses() as mocked:
        mocked.get(
            AROME_URL, status=400, payload=load_fixture("arome_out_of_domain.json")
        )
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {
...
async def test_user_flow_arome_only_location(hass: HomeAssistant) -> None:
    ...
    with (
        ...
        mocked.get(AROME_URL, payload=load_fixture("arome.json"))
        mocked.get(
            NOWCAST_URL, status=400, payload=load_fixture("nowcast_out_of_domain.json")
        )
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {
...
async def test_user_flow_cannot_connect(hass: HomeAssistant) -> None:
    ...
    with aioresponses() as mocked:
        mocked.get(re.compile(r".*"), status=500)
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"], USER_INPUT
        )
    assert result["type"] is FlowResultType.FORM

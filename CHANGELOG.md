...
- Change: a 429 and every 4xx are still answered once and reported as-is. Retrying a 429 spends what is left of a budget the API has just said is gone, and a rejected request is rejected identically three times; a 200 in an unexpected shape is a contract change rather than a blip. The retryable set is expressed as a `GeoSphereTransientError` branch of the exception hierarchy instead of a tuple at the call site, so adding a failure mode means deciding where it belongs. `GeoSphereServerError` is new and carries `status`; both remain `GeoSphereApiError` subclasses, so every existing `except` keeps working and the coordinator's log lines are unchanged
- Fix: a `ContentTypeError` is no longer reported as a connection failure, nor retried. It is an `aiohttp.ClientResponseError` and therefore an `aiohttp.ClientError`, so the blanket transport handler swept it up — logging "Error connecting to the GeoSphere API" for a 200 that arrived intact carrying HTML, which sends the reader after a network fault that never happened, and (once retries existed) spending the whole budget on an answer that will not change. The handler now splits response-level from transport-level: a fault mid-body (`ClientPayloadError`, `ServerDisconnectedError`) is not a `ClientResponseError` and stays retryable
- Fix: a 200 whose body is not JSON at all — a captive portal, a proxy error page, a maintenance page — raises `GeoSphereApiError` instead of escaping the hierarchy. The decode raises a `ValueError`, which is not an `aiohttp.ClientError` and was caught by nothing, so it travelled past every `except GeoSphereApiError` in the coordinators and surfaced as Home Assistant's generic "Unexpected error fetching ... data" traceback; in the config flow it escaped as an unknown exception rather than becoming `cannot_connect`. The 400 branch had guarded this case since it was written, which left the success path the odd one out rather than deliberately unguarded
- Fix: connection failures now name the exception class. `aiohttp.ClientConnectionError` and `ClientOSError` stringify to `""`, so the most frequent failure line in the log was also the least diagnosable — roughly 35 occurrences of `Error connecting to the GeoSphere API: ` with nothing after the colon. The message now falls back to the type name when `str(err)` is empty, and carries both when it is not
...
- Fix: `precipitation_1h` no longer reports a fraction of an hour as a full one. With INCA's hourly `RR` absent the coordinator summed the last four 15-min nowcast `rr` buckets — but the endpoint serves a single model run, clamped to that run's own t0 and published ~25-35 min after the analysis it is stamped for, so the buckets at/before now were never four. Measured 2026-09-11 against the live API at all three of the author's locations: an unbounded request returns **exactly one** bucket at/before now (11 buckets, ten of them forecasts), and even an anchored request reaches only the current run's start — 2 buckets at 11:42Z, 3 at 05:33Z. Every nowcast-fallback accumulation this integration ever published was therefore a 15-45 minute total labelled as an hour, under-reporting by up to 4× on exactly the degraded path it existed for. The field now reports `unknown` when INCA has none, as `precipitating` already does
- Change: the nowcast request carries a `start` of `NOWCAST_LOOKBACK` (45 min) before the 15-min floor of `now`. This repairs `RATE_LOOKBACK`, which takes the peak `rr` across the last 30 min specifically so a bucket that rounds to 0.0 in the gap between cells of an active storm cannot starve the `pouring` branch and the downpour override. With one bucket in the series that peak had nothing to widen and silently reduced to the matched bucket it exists to replace — the protection has never engaged in production. It engages in the test suite, which is why this went unnoticed: the recorded fixture was captured mid-bucket and happens to carry two stamps at/before the frozen clock
- Chore: reconstructing a genuine hourly total is possible — the t0 bucket of four consecutive runs via the undocumented `forecast_offset=0..3`, each run's t0 being its analysis — and was rejected at four requests per location per update against an endpoint returning six connection failures and three HTTP 503s in a single observed day. Recorded in [#35](https://github.com/slettmayer/ha-geosphere-next/issues/35) should the trade ever look different
...
- Change: "precipitating" now has a single definition, `condition.is_precipitating`, shared by the new sensor and by `derive_current_condition`. The two had drifted: the condition derivation counted the nowcast `pt` code **or** an instantaneous rate of at least `PRECIP_MIN_MM`, while the model field counted only `pt`
- Change: `is_precipitating` is tri-state, reporting `unknown` rather than `off` whenever nothing observed precipitation. Both sources are the 15-min nowcast, so both fall silent together — outside the Austrian grid (`CONF_HAS_NOWCAST` false, which skips INCA too since they share that grid) and on a failed nowcast fetch. Reporting `off` there is a confident "dry" nobody measured, next to a `precipitation_1h` already reporting `unknown`. The absence is now carried as `None` instead of being defaulted to 0.0 mm/h on the way through the coordinator
- Fix: a stale INCA `RR` no longer holds the derived condition on `rainy`. With the nowcast down, `RR` feeds the condition directly, and because `inca_latest` returns the newest value at any age and the cached slice is served on indefinitely while refreshes fail, rain that had stopped hours earlier kept the weather entity `rainy` under a 0 %-cloud sky — visibly contradicting the new `precipitating` sensor on the same device. `RR` now counts toward the condition only while it is younger than `INCA_RR_MAX_AGE_SECONDS` (2 h); past that the derivation falls through to cloud cover. The bound marks a slice that has stopped updating, not ordinary lag: INCA publishes ~30 min after the hour it analyses and serves the previous slice until the next appears, so the freshest `RR` in existence is routinely up to ~90 min old, and a tighter bound would reject the best data the source has for part of every cycle — flapping the condition once an hour through steady rain. `precipitation_1h` still reports the accumulation, which is a real measurement of a past hour
- Change: INCA's hourly `RR` is not accepted as evidence that it is precipitating *now*, only as the `precipitation_1h` measurement it is. It is an accumulation over the hour it is stamped for, while `inca_latest` returns the newest non-`None` value at any age and the coordinator serves a cached INCA slice for up to 55 minutes — indefinitely while refreshes keep failing. Read as an instantaneous rate it reported rain that had already stopped: 2.4 mm falling in the hour to 15:00 still read "wet" at 16:50, which on a `moisture` entity is what gets wired to closing an awning. The condition derivation, which must produce a string either way, still falls back to it

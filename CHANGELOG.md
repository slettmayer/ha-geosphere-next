...
- Change: "precipitating" now has a single definition, `condition.is_precipitating`, shared by the new sensor and by `derive_current_condition`. The two had drifted: the condition derivation counted either the nowcast `pt` code **or** an observed rate of at least `PRECIP_MIN_MM`, while the model field counted only `pt`. The rate half is what answers when the nowcast fetch fails and INCA's hourly `RR` is all that is left
- Change: `is_precipitating` is tri-state. Outside the Austrian grid (`CONF_HAS_NOWCAST` false) the coordinator skips the nowcast *and* INCA — they share that grid — so nothing observes precipitation at all, and both halves of the definition are silent. Reporting `off` there is a confident "dry" nobody measured, next to a `precipitation_1h` already reporting `unknown`. The absence is now carried as `None` instead of being defaulted to 0.0 mm/h on the way through the coordinator, and the binary sensor renders it `unknown`

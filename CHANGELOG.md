...
- Change: **behaviour change.** The convective-inhibition veto added in 0.9.0 no longer applies to *observed* precipitation. The current condition takes its rain from INCA and the nowcast — measurements — while CAPE and CIN come from AROME's forecast for that hour; inhibition answers "can convection get started?", which an observation has already settled. A modelled lid could therefore report a thunderstorm visibly in progress as plain `rainy`. The hourly forecast, which is forecast-driven end to end, keeps the full gate
...
- Fix: outside the nowcast grid, the Observation time sensor reports the AROME row's own stamp instead of `now`. There every field comes from the forecast hour in progress, stamped at the top of that hour, so at 14:59 the sensor claimed 14:59 for values describing 14:00 — the one sensor meant to expose staleness was hiding it
